########### Code for generating a mutation landscape waterfall plot. ###########

setwd("/Users/zhaoy2/Desktop/sc_project/maf_total")
library(ggplot2)
driver_file <- read.csv("/Users/zhaoy2/Desktop/1000_genomics/lung_drivers.csv")
driver_genes <- driver_file[driver_file$cancer_type == "luad" | driver_file$cancer_type == "lusc&luad",]$Gene_Symbol
head(driver_file)
driver_genes <- driver_genes[!driver_genes %in% c("STX2","MDM2","HOXB3","SFTPB")]
...
head(combined)
write.table(combined, file="Supp_table_mutations.txt", quote=F, sep="\t")
...
fabcolors_anno=list(Pathology=fabcolors)
